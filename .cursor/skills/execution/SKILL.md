...
1. **Check for Context**
   - If a memory MCP is available, query it with the project name and task topic
   - Review past decisions, blockers, and learnings that may affect execution
   - Check `/context/history/session_log.md` for recent decisions
...
3. **Persist Learnings**
   - If a memory MCP is available, store architectural decisions, bug fixes, and milestones
   - Update `/context/history/session_log.md` for significant decisions
